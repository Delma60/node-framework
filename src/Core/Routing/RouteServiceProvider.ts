...
export class RouteServiceProvider extends ServiceProvider {
    ...
    public async boot(): Promise<void> {
        ...
        const instantiateMiddleware = (MiddlewareClass: any) => {
            ...
            for (const route of webRoutes) {
                ...
                if (typeof server[method] === 'function') {
                    ...
                    if (rawHandler) {
                        ...
                        const routeMiddlewares = (route.routeMiddleware || []).map((alias: string) => {
                            const MiddlewareClass = middlewareConfig.aliases[alias];
                            if (!MiddlewareClass) throw new Error(`Middleware alias [${alias}] not found.`);
                            return instantiateMiddleware(MiddlewareClass);
                        });

                        // Inject: Group Middlewares -> Route Middlewares -> Handler
                        (server[method] as any)(route.uri, ...webGroupMiddlewares, ...routeMiddlewares, expressHandler);
                    }
                }
            }
        }
            ...
            for (const route of apiRoutes) {
                ...
                if (typeof apiRouter[method] === 'function') {
                    ...
                    if (rawHandler) {
                        ...
                        (apiRouter[method] as any)(route.uri, ...apiGroupMiddlewares, ...routeMiddlewares, expressHandler);
                    }
                }
            }
            server.use('/api', apiRouter);
